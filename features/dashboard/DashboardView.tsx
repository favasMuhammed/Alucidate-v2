...
import { User, SubjectData } from '@/types';
...
const AnimatedCounter: React.FC<{ value: number }> = ({ value }) => {
    ...
    useEffect(() => {
        ...
        let incrementTime = Math.max(16, totalDuration / end);
...
export const DashboardView: React.FC<DashboardViewProps> = ({ user, onLogout, onSwitchToAdmin, onSelectSubject }) => {
    ...
    const renderEmpty = () => (
        ...
        >
            <div className="w-24 h-24 mb-6 text-6xl flex items-center justify-center bg-raised rounded-full shadow-inner animate-[spin_12s_linear_infinite]">
                📖
            </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    <AnimatePresence>
                        {subjects.map((s, i) => {
                            ...
                            // Mock progress calculation for demo purposes
                            // Normally we'd calculate answered questions / total questions * 100
                            let totalQs = 0;
                            let answeredQs = 0;
                            if (s.structure?.children) {
                                s.structure.children.forEach(ch => {
                                    totalQs += 10; // Assume 10 Qs per chapter
